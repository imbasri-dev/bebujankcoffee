...
const search = (queryparams, hostAPI) => {
   return new Promise((resolve, reject) => {
      ...
      if (queryparams.sorting == "favorite") {
         query =
            "select pr.*, COALESCE(sum(tr.qty),0) as sold from product pr left join transactions tr on pr.id = tr.product_id where pr.delete_at is null GROUP BY pr.id ORDER by sold desc";
         link += `sorting=${queryparams.sorting}&`;
      }
...
const create = (body, file) => {
   return new Promise((resolve, reject) => {
      const query =
         "insert into product (name, category, size, price, stock, image, description) values ($1,$2,$3,$4,$5,$6,$7) returning *";
...
const drop = (params) => {
   return new Promise((resolve, reject) => {
      ...
      const query =
         "update product set delete_at = to_timestamp($1) where id = $2";
