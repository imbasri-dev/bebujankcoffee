...
const historyTransactions = (queryparams, token) => {
   return new Promise((resolve, reject) => {
      let query =
         "select transactions.id, users.email, product.name, product.image, transactions.qty, transactions.tax, transactions.method_payment, transactions.total, transactions.status from transactions inner join users on users.id = transactions.user_id inner join product on product.id = transactions.product_id where users.id = $1";

      let queryLimit = "";
      let link = `http://bebujankcoffee.vercel.app/coffee/transactions/history?`;
