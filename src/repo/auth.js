...
const login = (body) => {
   return new Promise((resolve, reject) => {
      ...
      const getPasswordsByEmailValues =
         "select id, email, passwords, role from users where email = $1";
      const getEmailActivation = "select status from users where email = $1";
         ...
         (err, response) => {
            ...
            postgreDb.query(getEmailActivation, [email], (err, response) => {
               ...
               if (response.rows[0].status === "pending") {
                  return reject({
                     err: new Error("please activated your account"),
                     statusCode: 401,
                  });
               }
